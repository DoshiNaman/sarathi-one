...
export default function HowItWorksPage() {
  return (
    <PageShell
      ...
      description="A prototype is only useful if you can tell what is real. This is the honest version: the problem, what we changed, what actually works, what is simulated, and how it could run at scale."
    >
      <div className="space-y-10">
        <section className="space-y-2">
          <h2 className="font-display text-2xl">Who has this problem</h2>
          <p className="text-sm leading-relaxed">{WHO}</p>
        </section>

        <section className="space-y-3">
          <h2 className="font-display text-2xl">What is hard about it today</h2>
          {TODAY.map((t) => (
            <Card key={t.problem}>
              <CardHeader>
        ...
        <section className="space-y-3">
          <h2 className="font-display text-2xl">What we changed</h2>
          {CHANGED.map((c) => (
        ...
        <section className="grid gap-4 sm:grid-cols-2">
          <Card className="border-success/40">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <CheckCircle2 aria-hidden className="text-success size-4" /> Works today
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border-warning/40">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <Drama aria-hidden className="text-warning size-4" /> Simulated
              </CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="text-muted-foreground list-disc space-y-1.5 pl-5 text-sm">
        ...
        <section className="space-y-3">
          <h2 className="font-display text-2xl">How this could work safely at scale</h2>
          {SCALE.map((s) => (
            <div key={s.heading}>
        ...
        <section className="text-muted-foreground rounded-lg border border-dashed p-4 text-sm">
          ...
          <p>
            Sarathi One is an independent hackathon prototype. It is not affiliated with, endorsed
            by, or connected to the Ministry of Road Transport and Highways, NIC, or Parivahan Sewa,
            and it uses no government logos or branding. For real services, use{" "}
